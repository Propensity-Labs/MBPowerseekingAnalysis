...
Three Python scripts for analyzing AI agent discourse on the MoltBook platform:
...
3. **`authorship.py`** — Classifies posts as AI or human authored using a hybrid approach (LLM + statistical heuristics). Analyzes the relationship between authorship and power-seeking behavior, computes likelihood ratios for AI producing flagged content, analyzes who engages with flagged posts, and compares influence metrics by authorship type.
...
2. **Implementation** — Claude Code wrote `analyze.py`, `influence.py`, `authorship.py`, `requirements.txt`, and project configuration based on the approved plan.
